...
| **Live chat**     | `GET\|POST /chat/conversations` 🔒 · `GET /chat/conversations/:id` 🔒 · `POST /chat/conversations/:id/messages` 🔒 · `POST /chat/ws-token` 🔒 · `WS /chat/stream` (receive-only, `?token=`) · guest: `POST /public/chat/conversations` · `GET /public/chat/conversations/:id` · `POST /public/chat/conversations/:id/messages` (guest Bearer) · admin: `/admin/chat/conversations` · `/admin/chat/conversations/:id{/messages,/close}` · `POST /admin/chat/ws-token` 👑                                                  |
